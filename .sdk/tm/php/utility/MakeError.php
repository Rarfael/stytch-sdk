...
class StytchMakeError
{
    public static function call(?StytchContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new StytchContext([], null);
        }
        $op = $ctx->op ?? new StytchOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new StytchResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof StytchError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new StytchError('', $msg, $ctx);
