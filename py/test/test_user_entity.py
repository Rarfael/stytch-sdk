    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set STYTCH_TEST_USER_ENTID JSON to run live")
        client = setup["client"]
...
def _user_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("STYTCH_TEST_USER_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("STYTCH_TEST_LIVE") == "TRUE":
        merged_opts = vs.merge([
            {
                "apikey": env.get("STYTCH_APIKEY"),
            },
            extra or {},
        ])
        client = StytchSDK(helpers.to_map(merged_opts))

    _live = env.get("STYTCH_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("STYTCH_TEST_EXPLAIN") == "TRUE",
        "live": _live,
