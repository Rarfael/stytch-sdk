...
describe("UserEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set STYTCH_TEST_USER_ENTID JSON to run live")
      return
    end
    ...
    assert.is_nil(err)
    assert.is_not_nil(user_ref01_data_dt0_loaded)
...
function user_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("STYTCH_TEST_USER_ENTID")
  ...
  if env["STYTCH_TEST_LIVE"] == "TRUE" then
  ...
  local live = env["STYTCH_TEST_LIVE"] == "TRUE"
  return {
    client = client,
    data = entity_data,
