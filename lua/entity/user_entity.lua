...
local UserEntity = {}
UserEntity.__index = UserEntity


function UserEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, UserEntity)
  self._name = "user"
  self._client = client
...
function UserEntity:get_name()
  return self._name
end


function UserEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return UserEntity.new(self._client, opts)
end
...
function UserEntity:mark_deleted()
  self._deleted = true
end


function UserEntity:deleted()
  return true == self._deleted
end


function UserEntity:data_set(args)
  if args ~= nil then
...
function UserEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function UserEntity:match_set(args)
  if args ~= nil then
...
function UserEntity:stream(action, args, callopts)
...
---@param reqmatch UserLoadMatch
---@param ctrl? table
---@return User
---@return string? err
function UserEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata UserUpdateData
---@param ctrl? table
---@return User
---@return string? err
function UserEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
---@param reqmatch UserRemoveMatch
---@param ctrl? table
---@return User
---@return string? err
function UserEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function UserEntity:_run_op(ctx, post_done)
  local utility = self._utility
