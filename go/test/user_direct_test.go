...
import (
	...
	sdk "github.com/voxgig-sdk/stytch-sdk/go"
...
func TestUserDirect(t *testing.T) {
	...
	t.Run("direct-load-user", func(t *testing.T) {
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-user", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type userDirectSetupResult struct {
	client *sdk.StytchSDK
	calls  *[]map[string]any
	live   bool
	idmap  map[string]any
}

func userDirectSetup(mockres any) *userDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"STYTCH_TEST_USER_ENTID": map[string]any{},
		"STYTCH_TEST_LIVE":    "FALSE",
		"STYTCH_APIKEY":       "NONE",
	})

	live := env["STYTCH_TEST_LIVE"] == "TRUE"

	if live {
		mergedOpts := map[string]any{
			"apikey": env["STYTCH_APIKEY"],
		}
		client := sdk.NewStytchSDK(mergedOpts)

		idmap := map[string]any{}
		if entidRaw, ok := env["STYTCH_TEST_USER_ENTID"]; ok {
		...
		return &userDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	client := sdk.NewStytchSDK(map[string]any{
	...
	return &userDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
