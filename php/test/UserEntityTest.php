...
class UserEntityTest extends TestCase
{
    ...
    {
        $testsdk = StytchSDK::test(null, null);
    ...
    public function test_stream(): void
    {
        ...
        $base = StytchSDK::test($seed, null);
        $seen = iterator_to_array($base->User(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = StytchConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = StytchSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set STYTCH_TEST_USER_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function user_basic_setup($extra)
{
    ...
    $client = StytchSDK::test($options, $extra);
    ...
    $entid_env_raw = getenv("STYTCH_TEST_USER_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["STYTCH_TEST_USER_ENTID"]);
    if ($idmap_resolved === null) {
        $idmap_resolved = Helpers::to_map($idmap);
    }

    if ($env["STYTCH_TEST_LIVE"] === "TRUE") {
        ...
        $client = new StytchSDK(Helpers::to_map($merged_opts));
    }
    ...
    return [
        ...
        "env" => $env,
        "explain" => $env["STYTCH_TEST_EXPLAIN"] === "TRUE",
        "live" => $live,
