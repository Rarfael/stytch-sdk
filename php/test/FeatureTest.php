...
class FtCtrl extends StytchControl
{
...
class FtClient
{
    ...
    private StytchUtility $utility;

    public function __construct(StytchUtility $utility, array $options)
    {
    ...
    public function get_utility(): StytchUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (StytchContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility = new StytchUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (StytchContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = StytchConfig::make_feature($name);
    ...
    private static function buildUrl(StytchSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof StytchError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new StytchResponse($fetched);
        }
    ...
    private function fail(StytchContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (StytchContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
