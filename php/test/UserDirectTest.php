...
class UserDirectTest extends TestCase
{
    ...
    public function test_direct_load_user(): void
    {
        ...
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-load-user", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
...
function user_direct_setup($mockres)
{
    ...
    if ($live) {
        ...
        $client = new StytchSDK($merged_opts);
        return [
            "client" => $client,
            "calls" => $calls,
            "live" => true,
