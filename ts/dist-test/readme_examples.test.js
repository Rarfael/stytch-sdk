...
const TEST_SEED = { "entity": { "user": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+StytchSDK\b/.test(code) ||
        /\bStytchSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+StytchSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/StytchSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
