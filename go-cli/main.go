// Command Stytch-cli is a CLI + REPL for the Stytch SDK,
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      STYTCH_APIKEY (api key), STYTCH_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: STYTCH_APIKEY carries the API key and
	// STYTCH_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("STYTCH_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("STYTCH_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewStytchSDK(opts)
