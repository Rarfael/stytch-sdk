...
boru-driven command-line client **and** interactive REPL for the Stytch
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/stytch-cli)
make build
...
./stytch-cli list user
./stytch-cli load 1 user            # {id:1} shorthand
./stytch-cli load '{id:1}' user       # explicit match map
./stytch-cli update '{name:"x"}' user

# 5. Override the API base URL for a single call
STYTCH_BASE=https://api.example.com ./stytch-cli list user
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/stytch-cli
   ```
...
```sh
./stytch-cli load 1 user          # scalar shorthand for {id:1}
./stytch-cli load '{id:1}' user     # explicit match map
```
...
```sh
./stytch-cli update '{id:1,name:"new"}' user
```
...
Run with no arguments to open a REPL (prompt `stytch>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `user`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/stytch-cli`. |
...
(`./stytch-cli <expr>`) and interactively (the REPL), and expressions compose the
