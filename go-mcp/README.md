...
[MCP](https://modelcontextprotocol.io) server exposing the Stytch SDK as
two agent tools — `stytch_list` and `stytch_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/stytch-mcp)
make build

# 2. Provide credentials via the environment
export STYTCH_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user stytch \
  -- /absolute/path/to/stytch-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./stytch-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// stytch_list: first page of records
{ "entity": "user" }
{ "entity": "user", "query": { } }

// stytch_load: one record by id
{ "entity": "user", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/stytch-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user stytch \
     -- "$PWD"/dist/*/stytch-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `stytch_list` and `stytch_load` tools now appear
   in new sessions. Ask the agent to *"list user using stytch"*
   and it calls `stytch_list` with `{"entity":"user"}`.
...
```sh
export STYTCH_APIKEY=sk_live_xxx            # API key
export STYTCH_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `stytch_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `stytch_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `STYTCH_APIKEY` | API key sent with every request. |
| `STYTCH_BASE` | Optional override of the API base URL. |
...
```sh
./stytch-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"stytch_load","arguments":{"entity":"user","query":{"id":1}}}}'
```
